...
```bash
# Use local database
export DATABASE_URL="postgresql://capp_user:REDACTED_DB_PASSWORD@localhost/capp_dev"
alembic upgrade head
```
...
```bash
# Use test database
export DATABASE_URL="postgresql://capp_user:REDACTED_DB_PASSWORD@localhost/capp_test"
alembic upgrade head
```
